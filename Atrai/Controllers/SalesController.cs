...
namespace Atrai.Controllers
{
    ...
    public class SalesController : Controller
    {
        ...
        ITransactionDetailsRepository transactionDetailsRepository, IProductTypeRepository productTypeRepository, IBrandRepository brandRepository, ISalesTagRepository salesTagRepository, IAdvanceTrasactionTrackingRepository advanceTransactionTrackingRepository,
        ...
        public JsonResult SomeAction(string[] SalesId, string CommandType)
        {
            try
            {
                ...
                else if (CommandType == "cntrlq")
                {
                    ...
                    var company = companyRepository.All().Where(x => x.Id == ComId).Include(x => x.Currency).FirstOrDefault();
        ...
        public void SendingFunction(string smstype, int Id, string CommandType)
        {
            try
            {
                ...
                if (smstype == "Invoice")
                {
                    ...
                    if (salesdata.CompanyList.isEmailSerivce == true)
                    {
                    ...
                    if (salesdata.CompanyList.isSMSService == true)
                    {
        ...
        [AllowAnonymous]
        [HttpGet]
        public IActionResult GetAgenciesJson()
        {
            try
            {
                var agency = _agencyRepository.All();
        ...
        [AllowAnonymous]
        [HttpGet]
        public IActionResult GetSalesJson()
        {
            try
            {
                var agency = _agencyRepository.All();
        ...
        [AllowAnonymous]
        public IActionResult CreateInvoiceQB(string type)
        {
            var ComId = HttpContext.Session.GetInt32("ComId");
        ...
        public IActionResult AddCustomer([FromBody] CustomerModel model)
        {
           ...
           .Select(x => new { x.Key, x.Value.Errors });
            ...
            if (model.Id == 0)
            {
                ...
                return Json(new { error = false, message = "Customer saved successfully", Id = model.Id });
            ...
            else
            {
                ...
                return Json(new { error = false, message = "Customer updated successfully", Id = model.Id });
        ...
        public IActionResult AddProduct([FromBody] ProductModel model)
        {
            if (model.Id == 0)
            {
                ...
                return Json(new { error = false, message = "Product saved successfully", Id = model.Id, PurchaseId = purcahseId });
            ...
            else
            {
                ...
                return Json(new { error = false, message = "Product updated successfully", Id = model.Id });
        ...
        public JsonResult GetProductInfo(int id)
        {
            try
            {
                ...
                var productDetails = product.Select(e => new
                {
                    ...
                    EANCode = e.EANCode,
                    CategoryId = e.CategoryId,
                    RuntimeLiveBalance = e.RunTimeLiveStock,
                ...
                return Json(new { success = true, data = productDetails, PurchaseId = purchaseCheck });
                //return Json("tesst" );
        ...
        public IActionResult InvoiceList(string Type, int IsRefresh = 1)
        {
            ...
            ViewBag.Consumption = accountHeadRepository.GetConsumptionDropdown();
            ViewBag.Supplier = supplierRepository.GetAllForDropDown();
            ...
            try
            {
                ...
                if (status != "0" && status != null)
                {
                    ...
                    if (status == "9999")
                    {
                        ...
                        salelist = salelist
                            .Where(s => s.ComId == ComId && !uniqueSalesIds.Contains(s.Id)
                        ...
                        if (item.Interval == "Daily")
                        {
                            if (item.NextDate == DateTime.Today && ((item.occurences > 0 && item.End_ == "After") || (item.PreviousDate != DateTime.Today)))
                            {
                                ...
                                if (item.End_ == "None" && item.PreviousDate != DateTime.Today)
                                {
                                    ...
                                    {
                                        item.NextDate = DateTime.Today.AddDays(item.Every_);
                                    }
                                    else
                                    {
                                        item.NextDate = item.PreviousDate?.AddDays(item.Every_);
        ...
        public IActionResult ProductPrice(int ProductId)
        {
            ...
            return Json(new { Success = 1, price = price , costprice = costprice, ex = "Data no found" });
        }
        ...
        public IActionResult ProductWiseTax(string taxes, string amounts, string criteria)
        {
            ...
            if(criteria == "inclusive")
            {
                ...
                {
                    var data = _salestaxRepository.All().Where(x => x.SalesTaxMasterId == taxIntArray[i]).ToList();
        ...
        public IActionResult ProductWiseTaxForExpense(string taxes, string amounts, string criteria)
        {
            ...
            foreach (var item in distinctDataOccurrences)
            {
                var listitem = new ProductTaxAmount();
                listitem.Nickname = item.Key;
                listitem.Amount = item.Value.Amount;
        ...
        public IActionResult ProductWiseChangeRate(string taxes, string amounts,string products, string criteria)
        {
            ...
            if(criteria =="inclusive")
            {
                for (int i = 0; i < taxIntArray.Length; i++)
                {
                    ...
                    var amount = amountIntArray[i];
                    //var afterCalculation = amount * (1 - data / 100) ?? 0;
                    var afterCalculation = amount / ((data / 100) + 1) ?? 0;
            ...
            try
            {
                ...
                var customerData = customerRepository.All().Where(x => x.Id == CustomerId)
                    .Select(x => new
                    {
                        ...
                        x.OverDueBalance,
                        x.OpBalance,
                        x.OpeningDate,/// = x.OpeningDate.ToString("dd-MMM-yyyy"),
        ...
        [AllowAnonymous]
        [HttpGet]
        public IActionResult ReceivePaymentQB(int? id)
        {
            var ComId = HttpContext.Session.GetInt32("ComId");
            ...
            ViewBag.DefaultInvoice = "DT-" + DateTime.Now.ToString("ddMMyyHHmmssfff");
        ...
        [AllowAnonymous]
        [HttpGet]
        public IActionResult LoadTransactionDetailsForCredit(int id)
        {
            var doctypeId = _docTypeRepository
                .All()
        ...
        [AllowAnonymous]
        public IActionResult GetReceivePaymentList(string? duration, int? customerId, int page = 1, decimal size = 5)
        {
            try
            {
                var ComId = HttpContext.Session.GetInt32("ComId");
