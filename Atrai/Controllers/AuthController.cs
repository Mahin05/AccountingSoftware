...
,
        ...
        public IActionResult StoreSetup()
        {
            ...
            var company = companyRepository.All().Where(x => x.UniqueCompanyId == comunqId).Include(x => x.Currency).Include(x => x.BusinessType).FirstOrDefault();
            ...
            if (storeinfo == null)
            {
                ...
                storesettings.CountryId = company.CountryId;
            ...
            HttpContext.Session.SetString("isSMSService", company.isSMSService.ToString());
            HttpContext.Session.SetString("isEmailSerivce", company.isEmailSerivce.ToString());
            HttpContext.Session.SetString("IsSignature", company != null ? company.IsSignature.ToString() : "false");


            HttpContext.Session.SetInt32("defaultcurrencyid", company.CountryId);
            HttpContext.Session.SetString("defaultcurrencyname", company.Currency.CurrencyShortName.ToString());
            ...
            var countryinfo = _countryRepository.All().Where(x => x.Id == company.CountryId).FirstOrDefault();
